...
{
    public class ReportBossController : Controller
    {
        // GET: ReportBoss
        public async  Task<ActionResult> ReportBoss()
        {
            return View(await CrearModelo());
        }

        public async Task<JsonResult> chart_Boss_Year(ReportVmBoss filtro)
        {
        ...
        public async Task<JsonResult>chart_Boss_Month(ReportVmBoss filtro)
        {
            Reporte rp = new Reporte();
            var datachart = await rp.graph_month_Boss(filtro);
            return Json(datachart,JsonRequestBehavior.AllowGet);
        }
        public async Task<JsonResult> chart_Boss_Day(ReportVmBoss filtro)
        {
            Reporte rp = new Reporte();
            var datachart = await rp.graph_Day_Boss(filtro);
            return Json(datachart, JsonRequestBehavior.AllowGet);
        }
        public async Task<JsonResult> chart_Boss_Year_Organizational(ReportVmBoss filtro)
        {
            Reporte rp = new Reporte();
