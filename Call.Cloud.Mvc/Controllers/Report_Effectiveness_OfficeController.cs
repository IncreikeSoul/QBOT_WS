...
{
    public class Report_Effectiveness_OfficeController : ApplicationController<LogOnModel>
    {
        // GET: Report_Effectiveness_Office
        ...
        private async Task<ListaReportOffice> CrearModelo(ReportsVm Item = null)
        {
            Reporte_Eficacia eficacia = new Reporte_Eficacia();
            EnterpriseLogica el = new EnterpriseLogica();

            if (Item == null)
                Item = new ReportsVm();
            var listareport = await eficacia.chart_Office_Vertical(Item);
            var listaempresa = await el.Retrieve(new Enterprise());

            return new ListaReportOffice(Item, listareport,listaempresa);
        }

        //Chart_Year_Office_Verticalmente
        public async Task<JsonResult> chart_Office_Vertical(ReportsVm filtro)
        {
            Reporte_Eficacia rp = new Reporte_Eficacia();
            var datachart = await rp.chart_Office_Vertical(filtro);
